...
def determine_output_dir(output_directory, dir_suffix = None, quiet = False):
    ...
    if not output_directory:
        ...
        if not quiet:
            print ("%sImages will be saved to temporary directory %s" % (style.prefix, temporary_directory))
    ...
    # and that it is writable. If it does not exist, try to create it.
        ...
        if not os.path.exists(output_directory):
            try:
                ...
                msg = "The output directory '%s' could not be created. " "Is the directory writable?" % output_directory
                raise IOError(msg)
